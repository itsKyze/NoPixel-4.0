...
function calculateScenesToDraw()
  ...
  local isPeekActive = exports['interactions']:IsToggled()
  for _, scene in pairs(activeScenes) do
    ...
    show = show and (#(scene.coords - playerCoords) < scene.distance) or false
    if show then
      local losObj = nil
      if not drawnObjs[scene.id] then
        RequestModel(`np_scene_dummy`)
        while not HasModelLoaded(`np_scene_dummy`) do
          Wait(0)
        end
        losObj = CreateObjectNoOffset(`np_scene_dummy`, scene.offsetCoords, 0, 0, 0)
    ...
    if UseNUIRender then
      if show and not nuiDrawnScenes[scene.id] then
        local coords = scene.coords

        local options = {
          distance = scene.distance,
          color = scene.color,
          background = scene.solid and NUI_OPTIONS.background.white or nil,
        }

        AddStaticDrawEntry(scene.id, scene.text, coords.x, coords.y, coords.z, options)

        nuiDrawnScenes[scene.id] = true

      elseif not show and nuiDrawnScenes[scene.id] then
        nuiDrawnScenes[scene.id] = nil

        RemoveDrawEntry(scene.id)
      end
    else
      if show then
