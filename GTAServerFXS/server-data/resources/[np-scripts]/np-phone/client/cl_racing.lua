...
RegisterUICallback("np-ui:racingGetLadder", function(data, cb)
  local res = exports["np-mkr-racing"]:getAllRaces()
  cb({ data = res, meta = { ok = true, message = 'done' } })
end)
...
RegisterUICallback("np-ui:racingPreviewRace", function(data, cb)
  exports["np-mkr-racing"]:previewRace(data.id, data.race.eventId)
...
RegisterUICallback("np-ui:racingCreateRaceTrack", function(data, cb)
  if RPC.execute("mkr_racing:isRaceNameTaken", data.raceName) == true then
    return cb({ data = {}, meta = { ok = false, message = "Track Name is Taken" } })
  end
  cb({ data = {}, meta = { ok = true, message = 'done' } })
  exports["np-mkr-racing"]:startRaceCreation(data)
end)

RegisterUICallback("np-ui:racingFinishCreateRaceTrack", function(data, cb)
  if RPC.execute("mkr_racing:isRaceNameTaken", data.raceName) == true then
    return cb({ data = {}, meta = { ok = false, message = "Track Name is Taken" } })
  end

  exports["np-mkr-racing"]:finishRaceCreation(data)
  cb({ data = {}, meta = { ok = true, message = 'done' } })
end)

RegisterUICallback("np-ui:racingCancelCreateRaceTrack", function(data, cb)
  exports["np-mkr-racing"]:cancelRaceCreation()
  cb({ data = {}, meta = { ok = true, message = 'done' } })
end)
...
RegisterUICallback("np-ui:racingJoinRace", function(data, cb)
  local canJoinOrErr = exports["np-racing"]:canJoinOrStartRace(data.race.vehicleClass, data.race.eventId)
...
RegisterUICallback("np-ui:racingEndRace", function(data, cb)
  if (data.race.viewType == "pending") then
    exports["np-mkr-racing"]:endRace(data.race.eventId)
  else
...
RegisterUICallback("np-ui:getEloLadder", function(data, cb)
  local res = RPC.execute("mkr_racing:getEloRatings", data.page, data.results_per_page)
  -- for key, value in pairs(res) do
  --   res[key].vehicleType = GetLabelText("VEH_CLASS_"..tostring(GetVehicleClassFromName(GetHashKey(res[key].model))))
  -- end
  cb({data=res, meta={ok=true, message='done'}})
end)

RegisterUICallback("np-ui:getEloLadderFilter", function(data, cb)
  local res = RPC.execute("mkr_racing:getEloRatingsFilter", data.page, data.results_per_page, data.alias)
  cb({data=res, meta={ok=true, message='done'}})
end)
...
AddEventHandler("mkr_racing:api:updatedState", function(state)
  ...
  exports['tablet']:SendAppEvent('underground', 'racing-update', data)
end)
...
AddEventHandler("mkr_racing:api:addedPendingRace", function(race)
  ...
  exports['tablet']:SendAppNotification('underground', 'New Race: ' .. race.name)
end)

AddEventHandler("mkr_racing:api:playerJoinedYourRace", function(characterId, name)
  exports['tablet']:SendAppNotification('underground', name .. ' joined your race')
end)

AddEventHandler("mkr_racing:api:playerLeftYourRace", function(characterId, name)
  exports['tablet']:SendAppNotification('underground', name .. ' left your race')
end)
