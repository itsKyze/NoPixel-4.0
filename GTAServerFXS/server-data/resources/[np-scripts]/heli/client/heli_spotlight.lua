...
AddEventHandler('onPlayerDropped', function(player)
    ...
    if (#activeSpotlightPlayers == 0) then spotlightThreadActive = false end
end)
    ...
    CreateThread(function()
        ...
        while spotlightThreadActive do
            for i, spotlightData in ipairs(activeSpotlightPlayers) do
                ...
                local coords = GetOffsetFromEntityInWorldCoords(vehicle, 0.0, 2.2, -1.5)
                ...
                DrawSpotLightWithShadow(coords, lerpedRotation, 200, 200, 205, 300.0, 0.75, 0.0, 8.0, 1.0, 0)
            end
            Wait(0)
        end
    end)
