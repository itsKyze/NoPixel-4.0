...
function heliSpot()
    ...
    if helicam and (IsPlayerInPolmav() or IsPlayerInNewsMav()) then
        if GetPedInVehicleSeat(heli, -1) ~= lPed then
            helispot = not helispot
            LocalPlayer.state:set('helispotlight', helispot, true)
        end
    end
end
...
function HelicamThread()
    ...
    CreateThread(function()
        ...
        AttachCamToEntity(cam, heli, 0.0, 2.2, -1.5, true)
        ...
        while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) do
            ...
            if helispot then
                ...
                DrawSpotLightWithShadow(camcoords, forward_vector, 200, 200, 205, 300.0, 0.75, 0.0, 8.0, 1.0, 0)
