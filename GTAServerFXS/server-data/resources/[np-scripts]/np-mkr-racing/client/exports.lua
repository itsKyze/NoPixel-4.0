...
function StartRaceCreation(options)
  if (getRaceCreationOptions().isCreatingTrack) then return end
  startRaceCreation(options)
end
exports("startRaceCreation", StartRaceCreation)


function FinishRaceCreation()
  return finishRaceCreation()
end
exports("finishRaceCreation", FinishRaceCreation)

function CancelRaceCreation()
  return cancelRaceCreation()
end
exports("cancelRaceCreation", CancelRaceCreation)

function CreateTrackPayload()
  return getRaceCreationOptions()
end
exports("createTrackPayload", CreateTrackPayload)
...
function previewRace(id, eventId)
  local race = races[id] or pendingRaces[eventId]
  if race == nil then return end

  race.checkpoints = race.checkpoints or RPC.execute("mkr_racing:getRaceCheckpoints", id)

  if id == 'night_event' then
    race.checkpoints = RPC.execute("mkr_racing:getRaceCheckpoints", eventId)

    race.start = race.checkpoints[1]
  end

  print(json.encode(race.checkpoints))
  ...
  if race.type ~= "PointToPoint" then
  ...
  if race.type == "Sprint" or race.type == "PointToPoint" then
...
function locateRace(id, eventId, reverse)
  ...
  if race.id == "random" or race.id == "night_event" then
...
function endRace(id)
  if id then
    RPC.execute("mkr_racing:endRace", id)
  elseif curRace then
