  ...
  while true do
    ...
    if camera then
      ...
      if not (IsPedSittingInAnyVehicle(lPed)) then
        Citizen.CreateThread(function()
          TriggerEvent('animation:PlayAnimation', 'camera')
      ...
      AttachCamToEntity(cam, lPed, 0.0, 1.1, 0.8, true)
      ...
      while camera and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and
        (IsPedUsingScenario(PlayerPedId(), "WORLD_HUMAN_PAPARAZZI") or IsEntityPlayingAnim(lPed, 'amb@world_human_paparazzi@male@idle_a', 'idle_c', 3)) and true do
