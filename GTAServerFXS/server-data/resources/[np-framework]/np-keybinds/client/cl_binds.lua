...
local excludesCommunicationBinds = false
AddEventHandler("np-binds:should-execute", function(shouldExecute, excludesCommunication)
  shouldExecuteBind = shouldExecute

  if excludesCommunication == nil then
    excludesCommunicationBinds = false
  else
    excludesCommunicationBinds = excludesCommunication
  end
end)

exports('registerKeyMapping', function(name, category, description, onKeyDownCommand, onKeyUpCommand, default, event, type, isCommunicationBind)
    if not default then default = "" end
    if not type then type = "keyboard" end
    if not isCommunicationBind then isCommunicationBind = false end
    ...
    RegisterCommand(cmdStringDown, function()
      if not shouldExecuteBind then
        if not isCommunicationBind then return end
        if not excludesCommunicationBinds then return end
      end
    ...
    if default and type == "keyboard" then
      if string.find(default, "§") or string.byte(default, 1) == 167 or string.byte(default, 1) == 194 then
        print(string.format("[np-keybinds] Sanitized invalid key %q for bind %q (%s)", tostring(default), tostring(cmdStringDown), tostring(desc or "")))
        default = "X"
      end
    end
