    ...
    comboZone:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
      if leftZones ~= nil then
        for i = 1, #leftZones do
          ...
          if leftZone.data?.isInteractionPoint then
            TriggerEvent("np-polyzone:interaction:exit", leftZone.name, leftZone.data, leftZone.center)
          else
            TriggerEvent("np-polyzone:exit", leftZone.name, leftZone.data, leftZone.center)
          end
        end
      end
      if enteredZones ~= nil then
        for i = 1, #enteredZones do
          ...
          local data = enteredZone.data

          if data?.isInteractionPoint then
            TriggerEvent("np-polyzone:interaction:enter", enteredZone.name, enteredZone.data, enteredZone.center)
          else
...
exports("ExpensiveGetZoneFromPoint", function(coords, names)
  if not comboZone or not coords then return {} end
