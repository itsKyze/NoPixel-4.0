...
function handleVolume(input)
    if almostEqual(0.0, input, 0.01) or input <= 0.0 then
        input = 0.0
        output = 0.0
    elseif almostEqual(1.0, input, 0.01) or input >= 1.0 then
        input = 1.0
        output = 1.0
    else
        output = input
...
function OverrideEndpoint(pEndpoint, enabled)
    ...
    Debug("[ENDPOINT] Override | Endpoint: %s | Enabled: %s", EndpointOverride or "N/A",
        _C(enabled, "enabled", "disabled"))
end
...
RegisterCommand("+mumble", function(_, pArgs)
    ...
    print((str):format(Config.version, isConnected, channel, proximityInfo, IsOnPhoneCall, phone, IsRadioOn,
        IsTalkingOnRadio, radio))
    ...
    if (pArgs[1] == "false" or pArgs[1] == "debug") then
        return
    end
