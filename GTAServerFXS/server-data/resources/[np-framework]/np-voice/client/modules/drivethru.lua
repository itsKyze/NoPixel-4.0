DTRadio, DTVolume, IsDTRadioOn, IsConnectedToDT, IsTransmittingToDT, DTIsHeadset = RadioChannel:new(120.6),
    Config.settings.dtVolume, true, false, false, false
...
function ConnectToDTFrequency(pIsHeadset)
    ...
    for serverId, active in pairs(subscribers) do
        if active then
            DTRadio:addSubscriber(serverId)
        end
    end
...
function AddDTSubscriber(pServerId)
    if DTRadio:subscriberExists(pServerId) then
        return
    end
...
function RemoveDTSubscriber(pServerId)
    if not DTRadio:subscriberExists(pServerId) then
        return
    end
...
function SetDTRadioPowerState(state)
    if Throttled("drivethru:powerState") then
        return
    end
    IsDTRadioOn = state
...
function SetDTRadioVolume(volume, pDisableNotification)
    ...
    if not pDisableNotification then
        TriggerEvent("DoLongHudText", ("New DriveThru volume %s"):format(math.floor(DTVolume * 100)), 1, 12000, {
            i18n = {"New DriveThru volume"}
        })
    end
...
function StartDTTransmission()
    if not IsDTRadioOn or not IsConnectedToDT or (DTIsHeadset and Throttled("drivethru:transmit")) or isDead then
        return
    end
...
function StopDTTransmission(forced)
    if not IsTransmittingToDT or DTTimeout then
        return
    end
...
function LoadDTModule()
    ...
    exports["np-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Push-To-Talk", "+transmitToDriveThru",
        "-transmitToDriveThru")
    RegisterCommand('+transmitToDriveThru', function()
        if not DTIsHeadset then
            return
        end
        StartDTTransmission()
    end, false)
    ...
    exports["np-keybinds"]:registerKeyMapping("", "DriveThru Headset", "On / Off", "+toggleDTRadioState",
        "-toggleDTRadioState")
    ...
    exports["np-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Up", "+increaseDTVolume",
        "-increaseDTVolume")
    RegisterCommand('+increaseDTVolume', IncreaseDTRadioVolume, false)
    RegisterCommand('-increaseDTVolume', function()
    end, false)

    exports["np-keybinds"]:registerKeyMapping("", "DriveThru Headset", "Volume Down", "+decreaseDTRadioVolume",
        "-decreaseDTRadioVolume")
    RegisterCommand('+decreaseDTRadioVolume', DecreaseDTRadioVolume, false)
    RegisterCommand('-decreaseDTRadioVolume', function()
    end, false)
