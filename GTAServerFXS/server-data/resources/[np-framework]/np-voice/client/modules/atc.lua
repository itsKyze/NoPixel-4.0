ATCRadio, ATCRadioVolume, IsATCRadioOn, IsConnected, IsTransmittingToATC = RadioChannel:new(118.0),
    Config.settings.atcVolume, true, false, false
...
function AddATCSubscriber(pServerId)
    if ATCRadio:subscriberExists(pServerId) then
        return
    end
...
function RemoveATCSubscriber(pServerId)
    if not ATCRadio:subscriberExists(pServerId) then
        return
    end
...
function SetATCRadioPowerState(state)
    if Throttled("atc:powerState") then
        return
    end
    IsATCRadioOn = state
...
function SetATCRadioVolume(volume, pDisableNotification)
    ...
    ATCRadioVolume = handleVolume(volume)
...
function IncreaseATCRadioVolume()
    local currentVolume = ATCRadioVolume
    SetATCRadioVolume(currentVolume + 0.1)
end
...
function StartATCTransmission()
    if not IsATCRadioOn or not IsConnectedToATC or Throttled("atc:transmit") or isDead or IsTransmissionDisabled("atc") then
        return
    end
...
exports("StartATCTransmission", StartATCTransmission)

function StopATCTransmission(forced)
    if not IsTransmittingToATC or ATCTimeout then
        return
    end
...
exports("StopATCTransmission", StopATCTransmission)
...
function LoadATCModule()
    ...
    RegisterCommand('+toggleATCRadioState', function()
        SetATCRadioPowerState(not IsATCRadioOn)
    end, false)

    RegisterCommand('-toggleATCRadioState', function()
    end, false)

    exports["np-keybinds"]:registerKeyMapping("", "ATC", "Volume Up", "+increaseATCRadioVolume",
        "-increaseATCRadioVolume")
    RegisterCommand('+increaseATCRadioVolume', IncreaseATCRadioVolume, false)
    RegisterCommand('-increaseATCRadioVolume', function()
    end, false)

    exports["np-keybinds"]:registerKeyMapping("", "ATC", "Volume Down", "+decreaseATCRadioVolume",
        "-decreaseATCRadioVolume")
    RegisterCommand('+decreaseATCRadioVolume', DecreaseATCRadioVolume, false)
    RegisterCommand('-decreaseATCRadioVolume', function()
    end, false)
