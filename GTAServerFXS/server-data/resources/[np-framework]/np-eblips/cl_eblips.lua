...
function IsDOC(pCallSign)
	if pCallSign and type(pCallSign) ~= "table" then
		...
		return sign == '9' -- 4.0 DOC Callsign starts with 9
	else
		return false
	end
end
...
function GetBlipSettings(pJobId, pCallSign, pDepartment, pSprite)
	...
	elseif pJobId == 'therapist' then
		settings.color = 23
		settings.heading =  true
		settings.text = ('Therapist | %s'):format(pCallSign)
	elseif pJobId == 'dib' then
		settings.color = 3
		settings.heading =  true
		settings.text = ('SASM | %s'):format(pCallSign)
    elseif pJobId == 'sama_director' then
        settings.color = 23
        settings.heading =  true
        settings.text = ('SAMA Director | %s'):format(pCallSign)
...
AddEventHandler("jobmanager:playerBecameJob", function(job, notify)
	...
	elseif job == "ems" or job == "sama_director" or job == "doctor" or job == "therapist" then
		DecorSetInt(PlayerPedId(), "EmergencyType", 2)
	elseif job == "doc" then
		DecorSetInt(PlayerPedId(), "EmergencyType", 3)
	elseif job == "dib" then
		DecorSetInt(PlayerPedId(), "EmergencyType", 4)
	else
		DecorSetInt(PlayerPedId(), "EmergencyType", 0)
	end
