...
local function EmitInOutEvent(pHandle, pIsInside)
    ...
    for _, options in pairs(events) do
        ...
        if pIsInside then
            if data?.isEntityInteraction then
                TriggerEvent("np-polyzone:entity:enter", options.name, data, pHandle)
            else
                TriggerEvent("np-polyzone:enter", options.name, data, pHandle)
            end
        else
            if data?.isEntityInteraction then
                TriggerEvent("np-polyzone:entity:exit", options.name, data, pHandle)
            else
...
local function toggleEntityDebug(enabled)
...
function RemoveEntityZone(name, id)
    for handle, zone in pairs(Zones) do
        if zone.name == name and zone.data.id == id then
...
if GetConvar("sv_environment", "prod") == "debug" then
    RegisterCommand("np-polyzone:debug-entity", function(src, args)
