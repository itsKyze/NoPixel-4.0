...
polyChecks = {
    ...
    courtRoom = { isInside = false, data = nil }
}
...
AddEventHandler("jobmanager:playerBecameJob", function(job, notify)
    ...
    if isTreasurer and job ~= "treasurer" then isTreasurer = false end
    ...
    if isTherapist and job ~= "therapist" then isTherapist = false end
    ...
    if job == "treasurer" then isTreasurer = true end
    ...
    if job == "therapist" then isTherapist = true end
    myJob = job
end)
...
AddEventHandler("np-polyzone:enter", function(zone, data)
    ...
    if zone == "court_room" then polyChecks.courtRoom = { isInside = true, polyData = nil } end
end)
...
AddEventHandler("np-polyzone:exit", function(zone)
    ...
    if zone == "court_room" then polyChecks.courtRoom = { isInside = false, polyData = nil } end
end)
