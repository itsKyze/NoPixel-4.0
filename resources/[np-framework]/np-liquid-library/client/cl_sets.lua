...
AddEventHandler("np-polyzone:enter", function(zone)
    if zone == "liquidlibrary:main" then
        ...
        local equal = isTableEqual(barEntitySets, result[1]) and ladyGemEntitySetEnabled == result[2] and
        stageLightEntitySetEnabled == result[3] and isTableEqual(northWingEntitySets, result[4])
...
AddEventHandler("np-liquidlibrary:reloadsets", function(barSets, gemSet, lightSet, northWingSets)
    ...
    local equal = isTableEqual(barEntitySets, barSets) and ladyGemEntitySetEnabled == gemSet and
    stageLightEntitySetEnabled == lightSet and isTableEqual(northWingEntitySets, northWingSets)
    if equal then return end
