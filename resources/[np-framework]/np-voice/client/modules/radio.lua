RadioChannels, IsRadioOn, IsTalkingOnRadio, RadioVolume, CurrentChannel, TransmissionTime = {}, false, false,
    Config.settings.radioVolume, nil
...
function SetRadioFrequency(pFrequency, pSendAppEvent)
    if CanUseFrequency(pFrequency, true) then
        ...
        if pSendAppEvent then
            exports["np-ui"]:sendAppEvent("radio", {
                value = pFrequency
            })
        end
...
local function ConnectToRadio(radioID, subscribers, canTalk)
    if RadioChannels[radioID] then
        return
    end

    local channel = RadioChannel:new(radioID)
    channel:setCanTalk(canTalk)
...
local function SetChannelCanTalk(radioID, canTalk)
    if not RadioChannels[radioID] then
        return
    end
    ...
    local channel = RadioChannels[radioID]
    channel:setCanTalk(canTalk)
    Debug("[Radio] Channel Can Talk | Radio ID: %s | Can Talk: %s", radioID, canTalk)
end
...
function StartTransmission()
    if not IsRadioOn or not CurrentChannel or Throttled("radio:transmit") or isDead or IsTransmissionDisabled("radio") then
        return
    end
    
    if RadioChannels[CurrentChannel.id]:getCanTalk() == false then
        TriggerEvent("DoLongHudText", "You are not allowed to talk on this channel.", 2)
        return
    end
    ...
    if not IsTalkingOnRadio then
        ...
        TriggerEvent("np:voice:transmissionStarted", {
            radio = true
        })
...
exports("StartTransmission", StartTransmission)

function StopTransmission(forced)
    if not IsTalkingOnRadio or RadioTimeout then
        return
    end
...
exports("StopTransmission", StopTransmission)

function IncreaseRadioVolume()
    local currentVolume = RadioVolume
    SetRadioVolume(currentVolume + 0.1)
end
...
function SetRadioVolume(volume, pDisableNotification)
    ...
    RadioVolume = handleVolume(volume)
    ...
    if not pDisableNotification then
        TriggerEvent("DoLongHudText", ("New volume %s"):format(math.floor(RadioVolume * 100)) .. "%", 1, 12000, {
            i18n = {"New volume"}
        })
    end
...
function SetRadioPowerState(state)
    if Throttled("radio:powerState") then
        return
    end

    if IsUnderwater and state then return end
...
function StartRadioTask(pIsInVehicle)
    Citizen.CreateThread(function()
        ...
        local isInVehicleAndFirstPersonDriver = pIsInVehicle and GetCamViewModeForContext(isInVehicle) == 4 and
                                                    GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) ==
                                                    PlayerPedId()
...
function increaseRadioChannel()
    ...
    TriggerEvent("DoLongHudText", ("New channel %s"):format(newChannel), 1, 12000, {
        i18n = {"New channel"}
    })
end
...
function DegradeRadio(pTransmissionLength)
    if not CurrentChannel or CurrentChannel.id <= 20.0 then
        return
    end

    local onRestrictedChannel = false
    for freq, entry in pairs(Config.frequencyConfig) do
        -- Config values are constants
        if freq == CurrentChannel.id and entry.restricted then
            onRestrictedChannel = true
            break
        end
    end
    if onRestrictedChannel or pTransmissionLength < 1000 then
        return
    end
...
function hasRadio()
    return exports["inventory"]:HasItem("radio", {
        quantity = 1,
        quality = 1
    }) or exports["inventory"]:HasItem("civradio", {
        quantity = 1,
        quality = 1
    })
end
...
function LoadRadioModule()
    ...
    exports["np-keybinds"]:registerKeyMapping("radio", "Radio", "Push-To-Talk", "+transmitToRadio", "-transmitToRadio",
        "CAPITAL", true, nil, true)
    RegisterCommand('+transmitToRadio', StartTransmission, false)
    RegisterCommand('-transmitToRadio', StopTransmission, false)

    exports["np-keybinds"]:registerKeyMapping("radio-secondary", "Radio", "Push-To-Talk (Secondary)",
        "+secondaryTransmitToRadio", "-secondaryTransmitToRadio", "", true, nil, true)
    ...
    RegisterCommand('-increaseRadioVolume', function()
    end, false)
    ...
    RegisterCommand('-decreaseRadioVolume', function()
    end, false)

    exports["np-keybinds"]:registerKeyMapping("", "Radio", "On / Off", "+toggleRadioState", "-toggleRadioState")
    RegisterCommand('+toggleRadioState', function()
        SetRadioPowerState(not IsRadioOn)
    end, false)
    RegisterCommand('-toggleRadioState', function()
    end, false)

    exports["np-keybinds"]:registerKeyMapping("", "Radio", "Channel Up", "+increaseRadioChannel",
    ...
    exports["np-keybinds"]:registerKeyMapping("", "Radio", "Channel Down", "+decreaseRadioChannel",
    ...
    RegisterNetEvent("np:voice:radio:cantalk")
    AddEventHandler("np:voice:radio:cantalk", SetChannelCanTalk)
