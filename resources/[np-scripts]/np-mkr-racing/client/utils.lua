...
function addWaypointAndBlip(checkpoints, checkpointNum, lookahead, isLastLap, isPointToPoint)
  clearBlips()
  SetWaypointOff()
  ClearGpsMultiRoute()
...
function findClosestVehicleNode(pos, searchRadius)
  local found, coords, heading = GetClosestVehicleNodeWithHeading(pos.x, pos.y, pos.z, searchRadius + 0.0, 1, 1, 1)
  if found then
    return { coords = coords, heading = heading }
  else
    return { coords = pos, heading = 0.0 }
  end
end

function generateVehicleNodeCheckpoints(startPos, endPos, numCheckpoints, searchRadius)
  local checkpoints = {}

  local dx = (endPos.x - startPos.x) / (numCheckpoints + 1)
  local dy = (endPos.y - startPos.y) / (numCheckpoints + 1)
  local dz = (endPos.z - startPos.z) / (numCheckpoints + 1)
  ...
  for i = 1, numCheckpoints do
    ...
    local nodeCheckpoint = findClosestVehicleNode(checkpoint, searchRadius)
    table.insert(checkpoints, nodeCheckpoint)
  end
