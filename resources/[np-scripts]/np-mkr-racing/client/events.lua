...
AddEventHandler("mkr_racing:startRace", function(race, countdown)
  ...
  for i, player in pairs(race.players) do
    playerAliases[player.racerTimestampId] = {player.alias, player.id}
  end
  local numOfPlayers = 0
  for i, _ in pairs(race.players) do
    numOfPlayers = numOfPlayers + 1
  end

  exports["racing"]:initRaceUI({
    playerAliases = playerAliases,
    numOfPlayers = numOfPlayers,
    startTime = race.startTime,
    laps = race.laps,
    numOfCheckpoints = race.numOfCheckpoints,
    driftMode = race.driftMode,
  })

  starting = true
  raceCountdownActive = true
  TriggerEvent("mkr_racing:api:startingRace", countdown)
  exports["racing"]:updateRacingUI({show = true, started = false})
  ...
  function changeFlagColor(checkpointColor)
    if config.checkpointUseMultiColorProp then
      SetObjectTextureVariation(checkpointObjects[1], checkpointColor)
      SetObjectTextureVariation(checkpointObjects[2], checkpointColor)
    end
  end

  -- SendNUIMessage({ type = 'countdown', start = 3 })
  exports["racing"]:updateRacingUI({countdown = true})
...
AddEventHandler("mkr_racing:updatePosition", function(playerDeltas)
  exports["racing"]:updateRacingUI({
...
AddEventHandler("mkr_racing:joinedRace", function(race)
  ...
  if race.id ~= "random" then
    spawnCheckpointObjects(race.start, config.startObjectHash, 2, 3)
  end
