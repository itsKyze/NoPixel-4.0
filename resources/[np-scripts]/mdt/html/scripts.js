...
$(document).ready(function () {
    ...
    window.addEventListener('mdt:enterEvidence', function (event) {
        $.post('https://mdt/enterEvidence', JSON.stringify(event.detail));
    });

    window.addEventListener('mdt:setGps', function (event) {
        $.post('https://mdt/setGps', JSON.stringify(event.detail));
    });
    ...
    window.addEventListener('message', async function (event) {
        ...
        if (item.event == 'hideWarrants') {
            const mdt = document.getElementById('mdt');
            mdt.contentWindow.postMessage(
                {
                    resource: 'mdt',
                    type: 'mdt:hideWarrants',
                    value: item.value,
                },
                '*',
            );
