...
RegisterNetEvent('np-rcvehicles:drone:spawn', function(pDroneSettings)
    if not pDroneSettings then return end
...
RegisterNetEvent('np-rcvehicles:drone:pilot', function(pDroneId)
    if not DRONES[pDroneId] then return end
    ...
    isPiloting = true
    TriggerEvent('DoLongHudText', 'Attempting to connect to drone.', 1)

    local timeout = GetGameTimer() + 5000
    while not NetworkHasControlOfNetworkId(CURRENT_DRONE.netId) and GetGameTimer() <= timeout and isPiloting do
        NetworkRequestControlOfNetworkId(CURRENT_DRONE.netId)
    ...
    if not NetworkHasControlOfNetworkId(CURRENT_DRONE.netId) then
        isPiloting = false
        TriggerEvent('DoLongHudText', 'Failed to request network control of drone.', 2)
        return
    end

    TriggerEvent('DoLongHudText', 'Connected to the drone.', 1)

    exports['inventory']:SetInventoryDisabled(true)
    exports['inventory']:SetActionBarDisabled(true)
    exports['hud']:sendAppEvent({ display = false })
    ...
    Citizen.CreateThread(function()
        ...
        setMethod(sf, 'SET_ZOOM_METER_IS_VISIBLE', false)
    ...
    Citizen.CreateThread(function()
        ...
        while isPiloting do
            ...
            for i = 1, #players do
                ...
                local dist = #(vector3(coords.x, coords.y, coords.z - 1) - CURRENT_DRONE.position)
                if (dist < 0.5 or IsEntityTouchingEntity(ped, CURRENT_DRONE.handle)) then
            ...
            if IsDisabledControlPressed(0, Keys['W']) then
                ...
                if CURRENT_DRONE.tilt > 45.0 then CURRENT_DRONE.tilt = 45.0 end
            end
            ...
            if IsDisabledControlPressed(0, Keys['D']) then
                ...
                if CURRENT_DRONE.roll < -30.0 then CURRENT_DRONE.roll = -30.0 end
            end
        ...
        exports['hud']:sendAppEvent({ display = true })
        ...
        local handle = CURRENT_DRONE.handle
        Citizen.CreateThread(function()
            local timeout = 5000
            while timeout > 0 and GetEntityHeightAboveGround(handle) > 0.5 do
                timeout = timeout - 100
                Wait(100)
            end
            if timeout <= 0 then return end

            SetEntityVelocity(handle, 0.0, 0.0, 0.0)
            SetEntityRotation(handle, 0.0, 0.0, GetEntityHeading(handle), 2)
            FreezeEntityPosition(handle, true)
        end)

        TriggerEvent('np-rc:droneExited', CURRENT_DRONE)
        RPC.execute('np-rcvehicles:leaveDrone', CURRENT_DRONE.id)
        if CURRENT_DRONE.crashed then RPC.execute('np-rcvehicles:crashDrone', CURRENT_DRONE.id) end
        CURRENT_DRONE = nil
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while isPiloting do
            if HasSoundFinished(flightLoop) then
                ...
                SetVariableOnSound(flightLoop, 'DroneRotationalSpeed', 0.0)
            end
...
RegisterNetEvent('np-rcvehicles:drone:destroy', function(pDroneId)
    if CURRENT_DRONE and CURRENT_DRONE.id == pDroneId then isPiloting = false end
    DRONES[pDroneId] = nil
end)
