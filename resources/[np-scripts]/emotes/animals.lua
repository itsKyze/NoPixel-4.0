...
local animsDog = {
    ...
    ["indicatelow"] = function(ped)
        local animDict = "creatures@rottweiler@indication@"
        local animation = "indicate_low"
    ...
    ["carleanleft"] = function(ped)
        ...
        loadAnimDict(animDict)
        TaskPlayAnim(ped, animDict, "lean_left", 1000.0, -1000.0, -1, 33, 0, false, false, false)
    end,
    ["carleanright"] = function(ped)
        local animDict = "creatures@rottweiler@in_vehicle@std_car"
        local playerVeh = GetVehiclePedIsIn(PlayerPedId(), false) 
        if playerVeh == 0 then return end
        ...
        loadAnimDict(animDict)
        TaskPlayAnim(ped, animDict, "lean_right", 1000.0, -1000.0, -1, 33, 0, false, false, false)
    end,
    ...
    ["fetchdrop"] = function(ped)
        local animDict = "creatures@rottweiler@move"
        local animation = "fetch_drop"

        if IsEntityPlayingAnim(ped, animDict, animation, 3) then
            ClearPedSecondaryTask(ped)
        else
            loadAnimDict(animDict)
            TaskPlayAnim(ped, animDict, animation, 1.0, -1, -1, 4, 0, false, false, false)
        end
    end,
    ["fetchpickup"] = function(ped)
        local animDict = "creatures@rottweiler@move"
        local animation = "fetch_pickup"
    ...
    ["getup"] = function(ped)
        local animDict = "creatures@rottweiler@getup"
        local animation = "getup_l"

        if IsEntityPlayingAnim(ped, animDict, animation, 3) then
            ClearPedSecondaryTask(ped)
        else
            loadAnimDict(animDict)
            TaskPlayAnim(ped, animDict, animation, 8.0, 8.0, -1, 4, 0, false, false, false)
        end
    end,
